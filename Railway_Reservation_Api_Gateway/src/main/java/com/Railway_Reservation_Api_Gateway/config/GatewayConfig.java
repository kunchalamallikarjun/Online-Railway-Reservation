...
public class GatewayConfig {
	...
	public RouteLocator myRoutes(RouteLocatorBuilder routeLocatorBuilder) {
		return routeLocatorBuilder.routes()
				...
				.route(p -> p.path("/user/signup").uri("http://localhost:8082"))
				.route(p -> p.path("/user/allusers").uri("http://localhost:8082"))
				.route(p -> p.path("/user/update").uri("http://localhost:8082"))
				.route(p -> p.path("/user/delete").uri("http://localhost:8082"))
				.route(p -> p.path("/user/alltrains").uri("http://localhost:8082"))
				.route(p -> p.path("/user/status").uri("http://localhost:8082"))
				.route(p -> p.path("/user/trainNo").uri("http://localhost:8082"))
				.route(p -> p.path("/user/route").uri("http://localhost:8082"))
